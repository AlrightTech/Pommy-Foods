...
export async function GET(request: NextRequest) {
  try {
    ...
    const totalRevenue = completedOrders?.reduce((sum, order) => sum + Number(order.final_amount || 0), 0) || 0;
    ...
    const dailySales = trendOrders?.reduce((acc: Record<string, number>, order) => {
